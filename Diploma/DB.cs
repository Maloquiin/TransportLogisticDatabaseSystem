...
namespace Diploma
{
    class DB
    {
        ...
        public static void AddAuto(AutoSQL std)
        {
            ...
            string sql = "INSERT INTO auto (DateDel, TypeCon, NumCon, id_client, Adress, Zpu, HoursPause, id_mkad, OptAdress, OverLoad, Expe, DateOplata, id_drivers) " +
        ...
        {
            string sql = "UPDATE auto SET DateDel = @DateDel, TypeCon = @TypeCon, NumCon = @NumCon, id_client = @id_client, Adress = @Adress, Zpu = @Zpu, HoursPause = @HoursPause, id_mkad = @id_mkad, OptAdress = @OptAdress, OverLoad = @OverLoad, Expe = @Expe, DateOplata = @DateOplata, id_drivers = @id_drivers WHERE id = @ID";
            MySqlConnection con = GetConnection();
            MySqlCommand cmd = new MySqlCommand(sql, con);
            cmd.CommandType = CommandType.Text;
            cmd.Parameters.Add("@DateDel", MySqlDbType.VarChar).Value = std.DateDel;
